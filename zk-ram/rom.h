    ...
    void Teardown_Batch(int &party, int block_size) {
        ...
        while (now_i < N+T) {
            ...
            for (int i = 0; i < block_size; i++, now_i++) {
                IntFp tmp_r = read_list[now_i].idx * A0 + read_list[now_i].version * A1 + read_list[now_i].val * A2 + X;
                IntFp tmp_w = write_list[now_i].idx * A0 + write_list[now_i].version * A1 + write_list[now_i].val * A2 + X;
